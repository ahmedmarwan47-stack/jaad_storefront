...
{qty_stepper(cart_bound=True)}
            ...
            <div class="delivery-note flex flex-wrap justify-between items-center gap-3 ps-5 pe-4 py-3 rounded-xl">
              ...
              <!-- min-h-6: this measured 83x20 on a phone, under WCAG 2.5.8's
                   24px floor. inline-flex so the floor applies to the box
                   rather than just the line, and the row it sits in is already
                   taller than 24 so nothing moves. -->
              <button type="button" data-open="location" class="inline-flex items-center min-h-6 font-semibold text-cta text-sm underline">تغيير المنطقة</button>
            </div>
      ...
      <div data-sticky-buybar aria-hidden="true"
        ...
        <!-- TWO ROWS ON A PHONE, one from sm up (Ahmed, 2026-09-01 mobile
             audit). As a single row at 375 the four children split 343px as
             title-block 115 / stepper 108 / CTA 96, and inside that 115 the
             name shares with the price sticker — leaving the product NAME 30px
             of a 137px string, i.e. about two letters and an ellipsis on every
             product page.

             `sm:contents` on the controls wrapper is what keeps the desktop
             layout untouched: it groups the stepper and the CTA into their own
             row on a phone, then removes itself from the box tree at sm so both
             become direct children of this flex row again, exactly as before.
             No duplicated markup, and no second set of hooks for the JS to
             bind. -->
        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 lg:gap-6 mx-auto px-4 max-w-[1536px] py-3">
          ...
          <!-- The phone's second row; `contents` from sm up, so this wrapper
               stops existing and the two controls sit in the main row again. -->
          ...
          <!-- `grow` only on the phone row, where it takes the width the
               stepper leaves; from sm it goes back to hugging its label. -->
          <button type="button" data-sticky-buy
                  class="grow sm:grow-0 bg-cta hover:bg-cta-hover px-5 xl:px-8 py-2.5 rounded-full font-semibold text-white text-sm whitespace-nowrap transition-colors shrink-0">اشتري الان</button>
          </div>
        </div>
      </div>"""
